...
import { increaseProductQuantity, decreaseQuantity, totalCal ,removeFromCart,clearCart} from '../redux/cart/cartSlice'
        ...
        <>
            <section className="py-24 relative">
                <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
                   ...
                   <button onClick={()=>{dispatch(clearCart())}} className="rounded-full group flex items-center justify-center focus-within:outline-red-500 my-3 flex gap-2">
                                            ...
                                            <div className='text-red-500'>Clear the cart?</div>
                                        </button>

                    {(items.length == 0) && <div className='font-normal text-base leading-7 text-gray-500 text-center mb-5 mt-6 max-lg:max-w-lg max-lg:mx-auto'>There is nothing in the cart</div>}
                    {items.map(product => {
                        // console.log(product)
                        return (
                            <div key={product.id} className="rounded-3xl border-2 border-gray-200 p-4 lg:p-8 grid grid-cols-12 mb-8 max-lg:max-w-lg max-lg:mx-auto gap-y-4">
                                <div className="col-span-12 lg:col-span-2 img box">
                                    <img src={product.url} alt="speaker image" className="max-lg:w-full lg:w-[180px] rounded-lg object-contain" />
                                </div>
                                <div className="col-span-12 lg:col-span-10 detail w-full lg:pl-3">
                                    <div className="flex items-center justify-between w-full mb-4">
                                        <h5 className="font-manrope font-bold text-2xl leading-9 text-gray-900">{product.name}</h5>
                                        <button onClick={()=>{dispatch(removeFromCart(product))}} className="rounded-full group flex items-center justify-center focus-within:outline-red-500">
                                            <svg width="34" height="34" viewBox="0 0 34 34" fill="none"
                                                xmlns="http://www.w3.org/2000/svg">
                                                <circle className="fill-red-50 transition-all duration-500 group-hover:fill-red-400"
                                                    cx="17" cy="17" r="17" fill="" />
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        placeholder="2" /> */}
                                            {(() => {
                                                ...
                                                return foundItem ? <span class="flex items-center justify-center border border-gray-200 rounded-full w-10 aspect-square outline-none text-gray-900 font-semibold text-sm py-1.5 px-3 bg-gray-100  text-center">{foundItem.count}</span> : <span class="flex items-center justify-center border border-gray-200 rounded-full w-10 aspect-square outline-none text-gray-900 font-semibold text-sm py-1.5 px-3 bg-gray-100  text-center">0</span>;
                                            })()}
                                        ...
                                        <h6  className="text-indigo-600 font-manrope font-bold text-2xl leading-9 text-right">:$ {(product.count * product.price).toFixed(2)}</h6>
                                    </div>
                                </div>
                            </div>
                        )
                    ...
                    <div className="flex flex-col md:flex-row items-center md:items-center justify-between lg:px-6 pb-6 border-b border-gray-200 max-lg:max-w-lg max-lg:mx-auto">
                        <h5 className="text-gray-900 font-manrope font-semibold text-2xl leading-9 w-full max-md:text-center max-md:mb-4">Total Qunatity</h5>

                        <div className="flex items-center justify-between gap-5 ">
                            
                            <h6 className="font-manrope font-bold text-3xl lead-10 text-indigo-600">{totalQuantity}</h6>
                        </div>
                    </div>
